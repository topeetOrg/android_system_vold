...
#define MASS_STORAGE_FILE_PATH0  "/sys/devices/platform/s3c-usbgadget/gadget/lun0/file"
#define MASS_STORAGE_FILE_PATH1  "/sys/devices/platform/s3c-usbgadget/gadget/lun1/file"
#define MASS_STORAGE_FILE_PATH2  "/sys/devices/platform/s3c-usbgadget/gadget/lun2/file"
...
int VolumeManager::shareVolume(const char *label, const char *method) {
    ...
    //To share the other storage, such as lun0,lun1,lun2
    int fd;
    char nodepath[255];
    snprintf(nodepath,
             sizeof(nodepath), "/dev/block/vold/%d:%d",
             MAJOR(d), MINOR(d));
    SLOGI("current devpath:%s, nodepath: (%s)", v->getMountpoint(), nodepath);

    char umspath[255] = "";
    if(strcmp(v->getMountpoint(), "/mnt/sdcard") == 0) {
        strcpy(umspath, MASS_STORAGE_FILE_PATH0);
    }
    else if(strcmp(v->getMountpoint(), "/mnt/sdcard2") == 0) {
        strcpy(umspath, MASS_STORAGE_FILE_PATH1);
    }
    else {
        strcpy(umspath, MASS_STORAGE_FILE_PATH2);
    }
    SLOGI("====shareVolume===current umspath:%s", umspath);

// if ((fd = open(MASS_STORAGE_FILE_PATH, O_WRONLY)) < 0) {
    if ((fd = open(umspath, O_WRONLY)) < 0) {
...
int VolumeManager::unshareVolume(const char *label, const char *method) {
    ...
    SLOGI("====unshareVolume===current umspath:%s", umspath);
    //if ((fd = open(MASS_STORAGE_FILE_PATH, O_WRONLY)) < 0) {
    if ((fd = open(umspath, O_WRONLY)) < 0) {	
